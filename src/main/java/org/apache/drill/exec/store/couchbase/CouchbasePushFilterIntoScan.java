...
public class CouchbasePushFilterIntoScan extends StoragePluginOptimizerRule {
  public static final StoragePluginOptimizerRule INSTANCE = new CouchbasePushFilterIntoScan();

  private CouchbasePushFilterIntoScan() {
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    CouchbaseGroupScan groupScan = (CouchbaseGroupScan)scan.getGroupScan();
    ...
    CouchbaseFilterBuilder hbaseFilterBuilder = new CouchbaseFilterBuilder(groupScan, conditionExp);
    CouchbaseScanSpec newScanSpec = hbaseFilterBuilder.parseTree();
    if (newScanSpec == null) {
      return; //no filter pushdown ==> No transformation.
    }

    final CouchbaseGroupScan newGroupsScan = new CouchbaseGroupScan(groupScan.getStoragePlugin(), newScanSpec, groupScan.getColumns());
  ...
  public boolean matches(RelOptRuleCall call) {
    final ScanPrel scan = (ScanPrel) call.rel(1);
    if (scan.getGroupScan() instanceof CouchbaseGroupScan) {
