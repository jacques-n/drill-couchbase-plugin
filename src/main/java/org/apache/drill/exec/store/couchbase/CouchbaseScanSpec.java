...
public class CouchbaseScanSpec {
  ...
  @JsonCreator
  public CouchbaseScanSpec(@JsonProperty("tableName") String tableName,
    ...
    if (filterString != null) {
      this.filter = CouchbaseUtils.parseFilterString(filterString);
    } else {
      this.filter = CouchbaseUtils.deserializeFilter(serializedFilter);
    }
  }

  public CouchbaseScanSpec(String tableName, byte[] startRow, byte[] stopRow, Filter filter) {
    this.tableName = tableName;
    this.startRow = startRow;
    this.stopRow = stopRow;
    this.filter = filter;
  ...
  public CouchbaseScanSpec(String tableName) {
    this.tableName = tableName;
  }
  ...
  public byte[] getSerializedFilter() {
    return (this.filter != null) ? CouchbaseUtils.serializeFilter(this.filter) : null;
  }
